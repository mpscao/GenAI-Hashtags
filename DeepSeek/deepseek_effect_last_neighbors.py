...
def generate_hashtags_deepseek(player_name, event, deepseek, previous_guesses = None, current_round = 1, effects = None, description = None):
    """
    Generate a hashtags for each player associated with the event

    Parameters:
    player_name (str): The player/agents in the game
    deepseek: the DeepSeek model used
    ...
    # for rounds after round 1, DeepSeek is asked to generate hashtags relevent to the event based on previous neighboring hashtags
    if current_round > 1:
        ...
        prompt = (f"In the experiment you are awarded with 1 point if you guess the same hashtag as your randomly-assigned neighbor, and 0 points if you don't guess the same hashtag. Your goal is to earn as many points as possible."
                  ...
                  f"{previous_table}\n"
                  f"\n"
                  f"Based on this information, the event provided to you in round 1, \n{event}\n, and the effects observed from round 1: {effects}, please guess a hashtag for this scenario with the goal of matching your randomly-assigned neighbor in this round."
                  f"Try not to use the hashtag you selected from last round, especially if you do not believe your next neighbor will have that hashtag.")
        #print(prompt)
    # otherwise for just round 1, DeepSeek generates hashtags relevant to the event without influence from neighbors
    else: 
    ...
    while retries < max_retries:
        try:
            ...
            if current_round == 1 or current_round == 40:
                ...
                effects_response = deepseek.generate_response(effects_prompt)
                ...
                description_prompt = (f"Write a tweet (DO NOT HAVE MORE than 140 characters for full response) about the event you just read \n\n {event} \n\n. Write in the following format: Tweet: your_140_character_response."
                                      f"Just write the your_1404_character_response, do not include your thoughts in the response")

                description_response = deepseek.generate_response(description_prompt)
                ...
                description = description[:150] if len(description) > 140 else description

                
            
            else:
                numbered_effects = effects or ["No effects provided"]
                description = description or "No Descrition provided"
            ...
            return chosen_hashtag, numbered_effects, description

        
        # in case of error, try to generate hashtag again as long as it is within the maximum number of retries
        except Exception as e:
            print(f"DeepSeek unable to generate hashtags, error: {e}.")
            retries += 1
      


        
    return f"#backup_{player_name}_{current_round}", effects or ["No effects provided"], description or "No description provided"
...
class HashtagMessagePassing(Environment):
    ...
    def get_neighbor_hashtags(self, player_name):
        ...
        if self.cur_round > 1:
            ...
            previous_round = self.cur_round - 1
        

            player_previous_hashtag = self.previous_hashtags[previous_round - 1].get(player_name)

            previous_neighbor = self.previous_neighbors[previous_round - 1].get(player_name)

            neighbor_previous_hashtag = self.previous_hashtags[previous_round - 1].get(previous_neighbor)

            
            previous_hashtags.append((previous_round, player_previous_hashtag, neighbor_previous_hashtag))
    ...
    def export_csv(self, filename = "agents_hashtags_effect_3.csv"):
        ...
        with open(filename, 'a', newline = '') as file:
            ...
            if write_header:
                writer.writerow(["Round", "Player", "Hashtag", "Effects", "Description", "Round Points", "Total Points"])
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self.cur_round == 1:
            chosen_hashtag, numbered_effects, description = generate_hashtags_deepseek(player_name, self.event, self.deepseek)
            ...
            chosen_hashtag, numbered_effects, description = generate_hashtags_deepseek(player_name, self.event, self.deepseek, previous_guesses, self.cur_round)
            ...
            chosen_hashtag, numbered_effects, description = generate_hashtags_deepseek(player_name, self.event, self.deepseek, previous_guesses, self.cur_round, effects_for_guess, description_for_guess)
    ...
    def coordination(self):
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig("effect_coordination_plot_3.png")
    ...
    def colormap(self):
        ...
        plt.savefig("effect_colormap_3.png")
