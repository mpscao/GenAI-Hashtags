...
def generate_hashtags_llama(player_name, event, llama, previous_guesses = None, current_round = 1, effects = None, description = None):
    """
    ...
    if current_round > 1:
        ...
        prompt = (f"In the experiment you are awarded with 1 point if you guess the same hashtag as your randomly-assigned neighbor, and 0 points if you don't guess the same hashtag. Your goal is to earn as many points as possible."
                  ...
                  f"{previous_table}\n"
                  f"\n"
                  f"Based on this information, the event provided to you in round 1, and the effects observed from round 1: {effects}, please guess a hashtag for this scenario with the goal of matching your randomly-assigned neighbor in this round."
    ...
    while retries < max_retries:
        try:
            ...
            if current_round == 1 or current_round == 40:
                ...
                description = description[:250] if len(description) > 140 else description
...
class HashtagMessagePassing(Environment):
    ...
    def export_csv(self, filename = "47_round_hasthags_graph_1.csv"):
    ...
    def step(self, player_name: str, action: str) -> TimeStep:
        ...
        if self.cur_round == 1:
            chosen_hashtag, numbered_effects, description = generate_hashtags_llama(player_name, self.event, self.llama)
            ...
            chosen_hashtag, numbered_effects, description = generate_hashtags_llama(player_name, self.event, self.llama, previous_guesses, self.cur_round)
            ...
            chosen_hashtag, numbered_effects, description = generate_hashtags_llama(player_name, self.event, self.llama, previous_guesses, self.cur_round, effects_for_guess, description_for_guess)
